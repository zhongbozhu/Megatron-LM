...
def wrap_model_chunks_with_ddp(
    ...
    *,
    ...
    For ``use_layer_wise_distributed_optimizer=True``, enables the DDP distributed-optimizer
    infrastructure and computes per-chunk layouts via
    :meth:`LayerWiseDistributedOptimizer.compute_full_param_layout`. LayerWise buffers use
    the padded or compact layout selected by ``ddp_config.use_layer_wise_param_layout``;
    sibling Adam buffers always use the byte-level DistributedOptimizer layout.
    ...
    Args:
        ...
        ddp_config: :class:`DistributedDataParallelConfig`. Mutated in place when
            ...
            field selects the padded (default) or compact LayerWise layout.
    ...
    if DP is DDP:
        if use_layer_wise_distributed_optimizer:
            ...
            # _ParamAndGradBuffer from ddp_config.use_layer_wise_param_layout: True keeps
            # LayerWise buffers on the reduce-scatter path, while False gives them the compact
