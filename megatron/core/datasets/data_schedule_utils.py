    ...
    if pp_group.size() > 2:
        if pp_group.rank() == 0:
            ...
            if pp_group.rank() != pp_group.size() - 1:
                ...
                if is_dynamic_cp:
                    local_cp_sizes = info_to_broadcast[
...
def reroute_samples_to_dcp_ranks(
    ...
    assert len(offset_values) == dp_size + 1, (
    ...
    assert len(batch) == len(local_ids), (
    ...
    def _build_layout(is_scalar):
        sample_numels = {
            gid: 1 if is_scalar else int(seq_len_by_gid[gid])
            for gid in range(offset_values[-1])
        }
        rank_numels = [
            sum(
                sample_numels[gid]
    ...
    for key in data_keys:
        ...
        assert local_tensor.numel() == rank_numels[dp_rank], (
...
def build_packed_microbatches(
    ...
    for i in range(num_micro_batches):
        ...
        new_sample = _pack_sequences(
            samples, lens_padded, lens_original, local_cp_size, dev
        )
        new_samples.append(new_sample)
...
def next_hdp_group_packing_aware(
    ...
    def cp_min_fn(seq_len: int) -> int:
        return dcp_gpus_needed(
            ...
            min_cp_size,
            valid_group_sizes=enabled_cp_sizes,
        )
