...
class LayerWiseDistributedOptimizer(ChainedOptimizer):
    ...
    def compute_full_param_layout(
        ...
        # The layout toggle affects Muon buffers only. Adam buffers retain byte-level sharding.
        ...
        for buffer_key, (group_params, param_indices) in buffer_groups.items():
            ...
            if buffer_key.is_managed_by_layer_wise_optimizer and not use_padded_layout:
                # Compact layout: attach param_indices so DDP's consistency check passes.
    ...
    def __init__(
        ...
        # Compact layout keeps whole-matrix ping-pong ownership (Newton-Schulz runs on whole
        ...
        # buffers instead gather variable-size lists of whole parameters into grad_data.
        ...
        # normally uses its synchronous allgather_params fallback.
        needs_variable_size_bucket_metadata = not self.use_buffer_param_sync and (
    ...
    def _shard_params_ping_pong(self, optimizers, dp_cp_size, expt_dp_size, model_chunks=None):
        """Assign whole parameters for compact layouts or construction without a DDP layout.
    ...
    def set_bucket_layerwise_params_list(self, model_chunks):
        ...
        Used by the compact variable-size all-gather path (``use_buffer_param_sync=False``).
    ...
    def step_with_ready_grads(self) -> bool:
        ...
        if not self.overlap_param_gather:
            if self.layerwise_param_sync_via_bucket_group:
                ...
                # layout OFF, the variable-size path uses grad_data.
