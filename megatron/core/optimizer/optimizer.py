                ...
                for i, param in enumerate(param_group['params']):
                    if param.requires_grad:
                        ...
                        if param.type() in ['torch.cuda.HalfTensor', 'torch.cuda.BFloat16Tensor']:
                            ...
                            # BF16/FP16 initializer on CPU. Seed the FP32 master from that value
                            # (not the lossy FP8 dequant), matching DistOpt, so primary-weight
                            # and non-primary-weight runs hold an identical master at iter 0.
                ...
                for model_param, main_param in zip(model_group, main_group):
                    if is_layerwise_fp8_param(model_param):
                        ...
                        if not getattr(model_param, '_layer_wise_fp8_gathered', False):
                            ...
                            # DP1/expert-DP1 parameters do not reach gather finalization;
                            # refresh blockwise columnwise storage after their local update.
                            post_all_gather_processing([model_param])
                    else:
