...
def uses_grad_buffer_for_fp8_param_gather(
    param: torch.Tensor, ddp_config: 'DistributedDataParallelConfig'
) -> bool:
    """Resolve whether an FP8 parameter gathers BF16 through reused gradient storage.

    MXFP8 follows its existing reuse flag for both optimizer owners. LayerWise-owned
    blockwise FP8 parameters always use BF16 staging; Adam's blockwise parameters keep
    their native FP8 gather path. Layout and tensor-parallel sharding do not affect this
    storage policy, and high-precision parameters never opt into it.

    Args:
        param: Model parameter, including its LayerWise ownership annotation.
        ddp_config: DDP configuration containing parameter-gather and MXFP8 reuse flags.

    Returns:
        Whether the parameter's all-gather temporarily reuses gradient storage.
    """
    if not ddp_config.fp8_param_gather:
        return False
    if is_mxfp8tensor(param) or is_grouped_mxfp8tensor(param):
        return ddp_config.reuse_grad_buf_for_mxfp8_param_ag
    return bool(
        getattr(param, 'is_managed_by_layer_wise_optimizer', False)
        and is_blockwise_float8tensor(param)
    )
