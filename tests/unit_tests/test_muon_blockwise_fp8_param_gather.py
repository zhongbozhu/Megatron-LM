...
@pytest.mark.parametrize("mxfp8_reuse", [False, True])
@pytest.mark.parametrize("is_muon", [False, True])
@pytest.mark.parametrize("kind", ["mxfp8", "blockwise", "bf16"])
def test_fp8_reuse_policy_is_layout_independent(
    monkeypatch, use_param_layout, fp8_param_gather, mxfp8_reuse, is_muon, kind
):
    """Blockwise reuse is implicit for Muon only; MXFP8 still requires its opt-in."""
    param = SimpleNamespace(kind=kind, is_managed_by_layer_wise_optimizer=is_muon)
    ddp_config = SimpleNamespace(
        use_layer_wise_param_layout=use_param_layout,
        fp8_param_gather=fp8_param_gather,
        reuse_grad_buf_for_mxfp8_param_ag=mxfp8_reuse,
    )
    monkeypatch.setattr(fp8_utils, "is_mxfp8tensor", lambda p: p.kind == "mxfp8")
    monkeypatch.setattr(fp8_utils, "is_grouped_mxfp8tensor", lambda p: False)
    monkeypatch.setattr(fp8_utils, "is_blockwise_float8tensor", lambda p: p.kind == "blockwise")

    expected = fp8_param_gather and (
        (kind == "mxfp8" and mxfp8_reuse) or (kind == "blockwise" and is_muon)
    )
    assert fp8_utils.uses_grad_buffer_for_fp8_param_gather(param, ddp_config) == expected
