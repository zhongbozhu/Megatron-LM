...
class TestDdpWrapFullParamLayout:
    ...
    @pytest.mark.parametrize("use_param_layout", [True, False])
    ...
    def test_layer_wise_layout_matches_traditional_model_entry(
        ...
        """Both layout choices resolve optimizer ownership before constructing DDP."""
        mock_ctx.return_value.__enter__ = Mock(return_value=None)
        mock_ctx.return_value.__exit__ = Mock(return_value=False)
        ...
        ddp_config = self._ddp_config(
            ...
            use_layer_wise_param_layout=not use_param_layout,
        )
        ...
        assert ddp_config.use_layer_wise_param_layout == use_param_layout
    ...
    @patch("torch.cuda.current_stream")
    @patch("torch.cuda.Stream")
    def test_layer_wise_layout_defaults_to_padded(
        self,
        mock_stream,
        mock_curr,
        mock_ctx,
        ...
        """The public default remains padded, including through the ModelBuilder path."""
        mock_ctx.return_value.__enter__ = Mock(return_value=None)
        mock_ctx.return_value.__exit__ = Mock(return_value=False)
        mock_layer_wise_optimizer.compute_full_param_layout.return_value = "LAYERWISE_LAYOUT"
        chunk, _param = self._make_chunk_with_params()
        ddp_config = self._ddp_config(
            use_distributed_optimizer=False, use_layer_wise_param_layout=False
        )
